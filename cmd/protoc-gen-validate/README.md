...
go get -d github.com/thoohv5/common/cmd/protoc-gen-validate
```

> #### 💡 Yes, our go module path is `github.com/thoohv5/common/cmd/protoc-gen-validate` **not** `bufbuild` this is intentional.
...
protoc \
  -I . \
  -I ${GOPATH}/src \
  -I ${GOPATH}/src/github.com/thoohv5/common/cmd/protoc-gen-validate \
  --go_out=":../generated" \
...
PGV comes with a [Dockerfile](/Dockerfile) for consistent development tooling and CI. The main entrypoint is `make` with `quick` as the default target. This repo should be volumed into `/go/src/github.com/thoohv5/common/cmd/protoc-gen-validate` for the proper behavior.
...
docker run --rm \
  -v $(PWD):/go/src/github.com/thoohv5/common/cmd/protoc-gen-validate \
